...
export class WebGLInterface {
  ...
  assertWebGL2() {
    if (this.ctxVersion < 2) {
      throw new Error(
        "WebGL2 procedure called in a canvas without a WebGL2 context"
      );
    }
  }
  getNewId(table) {
    ...
    return {
      ...
      GetProgramInfoLog: (program, buf_ptr, buf_len, length_ptr) => {
        ...
        if (buf_len > 0 && buf_ptr) {
          ...
          this.mem
            ...
            .set(new TextEncoder().encode(log));
      ...
      GetShaderiv: (shader, pname, p) => {
        if (p) {
          if (pname == 35716) {
            ...
            let param = this.ctx.getShaderParameter(
              this.shaders[shader],
              pname
            );
      ...
      GetUniformLocation: (program, name_ptr, name_len) => {
        ...
        if (name.indexOf("]", name.length - 1) !== -1) {
          ...
          if (
            arrayIndex.length > 0 &&
            (arrayOffset = parseInt(arrayIndex)) < 0
          ) {
            return -1;
          }
      ...
      IsFramebuffer: (framebuffer) =>
        this.ctx.isFramebuffer(this.framebuffers[framebuffer]),
      ...
      ReadnPixels: (x, y, width, height, format, type, bufSize, data) => {
        this.ctx.readPixels(
          ...
          format,
          type,
          this.mem.loadBytes(data, bufSize)
        );
      },
  ...
  getWebGL2Interface() {
    return {
      ...
      InvalidateFramebuffer: (target, attachments_ptr, attachments_len) => {
        this.assertWebGL2();
        let attachments = this.mem.loadU32Array(
          attachments_ptr,
          attachments_len
        );
      ...
      InvalidateSubFramebuffer: (
        ...
        this.ctx.invalidateSubFramebuffer(
          target,
          attachments,
          x,
          y,
      ...
      DrawElementsInstanced: (mode, count, type, offset, instanceCount) => {
        this.assertWebGL2();
        this.ctx.drawElementsInstanced(
          mode,
          count,
          type,
          offset,
      ...
      TransformFeedbackVaryings: (
        ...
        this.ctx.transformFeedbackVaryings(
          this.programs[program],
          varyings,
          bufferMode
        );
      ...
      GetUniformBlockIndex: (
        program,
        uniformBlockName_ptr,
        uniformBlockName_len
      ) => {
        this.assertWebGL2();
      ...
      GetActiveUniformBlockName: (
        ...
        this.mem
          .loadBytes(buf_ptr, buf_len)
          .set(new TextEncoder().encode(name));
        this.mem.storeInt(length_ptr, n);
      },
      UniformBlockBinding: (
        program,
...
export async function runWasm(
  ...
  if (exports.memory) {
    if (wasmMemoryInterface.memory) {
      console.warn(
        "WASM module exports memory, but `runWasm` was given an interface with existing memory too"
      );
    }
